...
train_loader = DataLoaderLite(B, T, process_rank=ddp_rank, num_process=ddp_world_size)
...
for i in range(max_steps):
    ...
    for micro_step in range(grad_accum_steps):
        ...
        if ddp:
            model.require_backward_grad_sync = (micro_step == grad_accum_steps - 1) # for gradient accumulation
        loss.backward() # remeber the issue loss reduction mean
    if ddp:
        torch.distributed.all_reduce(loss_accum) # sum the loss across all processes
